...
% Instituto Superior T閏nico, Outubro, 2007
...
% ------------  TV experiments ---------------------
K=fft2(B);
KC=conj(K); %conj为求复共轭的函数

% handle functions for TwIST
%  convolution operators
A = @(x) real(ifft2(K.*fft2(x))); %real返回实部
AT = @(x) real(ifft2(KC.*fft2(x)));

% denoising function; %TV去噪和正则化
tv_iters = 5;
